...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		// Checkbox
		case "checkbox":
			$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="checkbox on-of of-input" type="checkbox" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" '. checked( $val, 1, false) .' />';
		...
		case 'typography':	
			...
			foreach ( $styles as $key => $style ) {
				...
				$font_choise=  esc_attr( $key ) ;
			}
			$output .= '</select>';
		
			// Font Face
			$output .= '<select class="of-typography of-typography-face theme-google-font-selector" name="' . esc_attr( $option_name . '[' . $value['id'] . '][face]' ) . '" id="' . esc_attr( $value['id'] . '_face' ) . '" style="margin-top:20px;">';
			
			//$faces = array_merge (of_recognized_font_faces_d(),of_recognized_font_faces());
			$faces = of_recognized_font_faces();
			foreach ( $faces as $key => $face ) {
				
				$output .= '<option value="' . esc_attr($key) . '" ' . selected( $typography_stored['face'],  $key , false ) . '>' . esc_html( $face ) . '</option>';
				$font_choise = str_replace ('+',' ', $typography_stored['face']);
				$font_demo = $typography_stored['face'];
				
			}			
			
			$output .= '</select>';	
			
			$output .= '<link href="http://fonts.googleapis.com/css?family='.$font_demo .'" rel="stylesheet" type="text/css" class="'.$value['id'].'_face-temp-header-font" />';							
			$output .= '<div class="clear"></div><div id="'.$value['id'].'_face-preview" class="theme-header-font-preview" style="font-family: '.$font_choise.'; margin-top:10px;">';
		...
		case "heading":
			...
			$menu .= '<li><a id="'.  esc_attr( $jquery_click_hook ) . '-tab" class="nav-tabs" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '">' . esc_html( $value['name'] ) . '</a></li>';
