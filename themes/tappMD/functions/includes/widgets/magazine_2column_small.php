 ...
 *Magazine Widget 3
 */
add_action('widgets_init', 'csc_Magazine_2_small_column_widgets');

function csc_Magazine_2_small_column_widgets()
{
	register_widget('csc_2_small_Magazine_Widget');
}

class csc_2_small_Magazine_Widget extends WP_Widget {
	
	function csc_2_small_Magazine_Widget()
	{
		$widget_ops = array('classname' => 'csc_magazine_2_small', 'description' => '2 Column and small magazine recent posts widget.');

		$control_ops = array('width' => 250, 'height' => 350,'id_base' => 'csc_magazine-widget_2_small');

		$this->WP_Widget('csc_magazine-widget_2_small', CSC_NAME .' Magazine 2 Col and small', $widget_ops, $control_ops);
	}
	...
	<?php } ?>
		...
		?>
         <?php 
		$ids = $args['widget_id'] ;
		echo '<style>  #'.$ids.' .pull-col .widget-title h3,#'.$ids.' a.all_cat.color_2:hover,#'.$ids.' a.rss_cat.color_2:hover,#'.$ids.' .scorehome.color_2,#'.$ids.' span.icon.color_2,#'.$ids.' .news-info div,#'.$ids.' .scorehomebig{ background-color:'.$background_2.';}</style>';
	...
	{
		$defaults = array('title' => 'Recent Posts','background' => '','background_2' => '', 'post_type' => 'all', 'categories' => 'all', 'posts' => 4, 'title_2' => 'Recent Posts', 'post_type_2' => 'all', 'categories_2' => 'all', 'posts_2' => 4 );
